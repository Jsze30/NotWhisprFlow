...
There is no test target. Requires Xcode command-line tools, macOS 13+. Built via SwiftPM (`Package.swift`) as a single executable target; the Makefile wraps the binary in an `.app` bundle so AppKit + TCC permissions work correctly. Every rebuild should be `make && ditto WhisperFlow.app /Applications/WhisperFlow.app` so the installed copy stays in sync.
